...
namespace pfilter
{
    ...
    cv::Vec3d MotionModel::update(cv::Vec3d u_t0, cv::Vec3d u_t1, cv::Vec3d x_t0)
    {
        ...
        std::normal_distribution<double> rot1_dist(
            0, std::sqrt(alpha_1 * (std::pow(odo_rot1_delta, 2)) + alpha_2 * (std::pow(odo_trans_delta, 2))));
        std::normal_distribution<double> trans_dist(
            0,
            std::sqrt(alpha_3 * (std::pow(odo_trans_delta, 2)) +
                      alpha_4 * (std::pow(odo_rot1_delta, 2) + std::pow(odo_rot2_delta, 2))));
        std::normal_distribution<double> rot2_dist(
            0, std::sqrt(alpha_1 * (std::pow(odo_rot2_delta, 2)) + alpha_2 * (std::pow(odo_trans_delta, 2))));
    ...
    bool MotionModel::isMoving(const cv::Vec3d& u_t0, const cv::Vec3d& u_t1)
    {
